...
/*
        Need to be able to replay the hand, so 
...
struct hand_context
{
        struct player_type{
                ...
                {}
                auto stack()const{ return stack_; }
                auto dist()const{ return stack_; }
                auto const& name()const{ return name_; }
        private:
                double stack_;
                std::string name_;
                hand_distribution dist_;
        };
...
private:
        double sb_;
        double bb_;
        std::vector<player_type> players_;
};
...
struct event_decl{
        ...
        virtual ~event_decl()=0;
        virtual void apply()const=0;
        event_decl* parent(){ return parent_; }
        Type type()const{ return type_; }

        explicit event_decl(Type t, event_decl* p)
                :type_{type}
                ,parent_{p}
        {}
private:
...
struct player_move_decl : event_decl{
        auto id()const{ return id_; }
        auto const& dist()const{ return dist_; }
private:
        size_t id_;
        hand_distribution dist_;
};

struct deal_btn : event_decl{
        void apply(game_tree_node& node)const override{
                game_tree_node child(node);
                child.dealer().deal_btn(n_);
...
struct round_of_betting : event_decl{
        void apply(holdem_game_context* ctx, game_tree_node* node)const override{
                std::vector<game_tree_node*> stack;
                ...
                for(;;){
                        ...
                        auto player_id = ptr->active();
                        auto s = ctx->player_move( ptr );

                        auto moves = s.compile();
                        for( auto const& m : moves ){
                                auto child = ctx->game_logic( node, m );
                                ptr->add_child(child);
...
struct game_tree_node{
        std::vector<game_tree_node*> children_;
};

struct start_node{
        start_node(hand_context ctx,
};
...
struct bar{
        void bar(){
                auto strat = player->foo(last);

                std::vector<state> children;
                for( auto const& sub : strat){
                        switch(sub.action()){
                        case PlayerAction_Fold:
                                children_.emplace_back(new fold_node(ctx(), sub.range()));
                                break;
                        case PlayerAction_Push:
                                children_.emplace_back(new push_node(ctx(), sub.range()));
                                break;
                }
        }
};
...
struct street_decl{
private:
        std::vector< event_decl* > vec_;
};


struct betting_round{};

void other_idea(){

        struct push_fold_visitor : holdem_game_context{
                hand_strategy player_action(hand_distribution const& dist)const override{
                        hand_strategy s;
                        s.default_.fold();
                        return std::move(s);
                }
        ...
        hand_decl decl(0.5, 1);
        decl.push_player(10, "Hero");
        decl.push_player(10, "Villian");

        street_decl preflop;
        preflop.push( new deal_btn{0} );
        preflop.push( new deal_hands{new sub_hand{"AA"}, new sub_enum{}});
        //preflop.push( new deal_hands{"AKo", "TT"});
        preflop.push( new post_sb{} );
        preflop.push( new post_bb{} );
        preflop.push( new round_of_betting{} );
        
        street_decl river;
        river.push( new round_of_betting{} );

        game_decl gd;
        gd.push( new street{preflop} );
        gd.push( new deal_board{"A2Kd5h8c9c"} );
        //gd.push( new enum_board{} );
        gd.push( new street{river} );

        auto engine = game_engine::compile(gd);
        engine->bar( decl, v );
