...
namespace support{
        ...
        struct AnyContext{
                ...
                typename std::decay_t<Decl>::ReturnType ValueOrThrow(Decl const& decl){
                        ...
                        for(;!!head;head = head->parent_){
                                auto iter = std::lower_bound(head->items_.begin(), head->items_.end(), key,
                                                             [](auto l, auto r){ return *l < r; } );
                                if( iter != head->items_.end() ){
                                        return decl.Cast((**iter).data);
                                }
                        }
        ...
        struct ValueDecl{
                ...
                void CreateOrThrow(AnyContext* ctx, AnyContext::Maker const& maker)const{
                        maker.Create(key_, std::make_shared<T>(init_));
                }
